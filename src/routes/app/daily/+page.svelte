<script lang="ts">
  ...
  function sortTickets(tickets: Awaited<ReturnType<typeof daily.list>>) {
  ...
  let tickets = $state(sortTickets(await daily.list()))
...
<div class="flex items-center gap-2 mb-4">
  ...
  <Modal triggerClass={[buttonVariants(), "ml-auto"]} title="Daily toevoegen">
    {#snippet trigger()}
      ...
      Toevoegen
    {/snippet}
    <form {...forms.apply(daily.add)}>
      <Field.Set>
        <Field.Group>
          <Field.Field>
            <Field.Label for="ticket">Ticket</Field.Label>
            <Input {...daily.add.fields.ticket.as("text")} id="ticket" type="number" placeholder="285862"/>
            {#if daily.add.fields.ticket.value() !== ""}
              <Field.Description>
                {#await wticket.getTicket(Number(daily.add.fields.ticket.value())) then ticket}
                  {ticket.description}
                {:catch _}
